import { Button, Card, Flex, InputNumber, Layout, Space, Switch, Typography } from 'antd'
...
function App() {
  ...
  const handleLevelUp = (indexItem: number, levels: number = 1) => {
    store.updateItems(store.items.map((item, index) => index === indexItem ? { ...item, level: item.level + levels } : item))
  }
  ...
  const chanceLevelUp = (indexItem: number, levels: number = 1) => {
    ...
    if (store.items[indexItem].level > 99) {
      if (random >= 99) { handleLevelUp(indexItem, levels) }
    } else {
      if (random >= store.items[indexItem].level) { handleLevelUp(indexItem, levels) }
    }
  }
  ...
  return (
    <Layout>
        ...
        <Flex gap='large' vertical>
          ...
          <Flex gap='small' align='space-between'>
            <Card title="Панель">
              <Flex vertical gap='small'>
                <Space>
                  <Button onClick={() => makeWin(10)}>Победа</Button>
                ...
                <Space>
                  <Switch checkedChildren="Старт" unCheckedChildren="Стоп" />
          ...
          <Flex gap='small'>
            {store.items.map((item, index) =>
              <Card key={item.name} title={item.type === 'skill' ? 'Умение' : 'Оружие'} extra={<Button onClick={() => handleLevelUp(index, 25)}>+ 25 уровеней</Button>}>
                <Flex vertical>
                  <Typography.Text>Ур. {item.level}. Шанс {100 - item.level}%</Typography.Text>
      ...
      <Layout.Footer>Телеграмм</Layout.Footer>
    </Layout>
  )
}
